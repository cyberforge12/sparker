...
class Validate {
  ...
  val ext_vals = Map("number_card_recepient" -> ValidateConfig("[X0-9]{16,20}", nullable = true, Set()), "payer_card_number" -> ValidateConfig("[X0-9]{16,20}", nullable = true, Set()),
    ...
    "issue_date_card_owner" -> ValidateConfig("\\d{2}.\\d{2}.\\d{4}\\s+\\d{1,2}:\\d{2}:\\d{2}", nullable = true, Set()))

  def validateField(value: String, vConf: ValidateConfig): Boolean = {
    if ((isNull(value) && vConf.nullable) || (!isNull(value) && !vConf.nullable)) {
  ...
  def isNull(s: String) = {
    if (s.equals("null") || s.equals("NULL")) true
    else false
  }
