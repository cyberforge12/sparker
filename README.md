...
3. в конструкция типа else logger.info("Dataframe schema successfully validated") - было бы неплохо всегда добавлять скобки {}, это полезная привычка, не отнимает много времени, наверно все IDE умеют при вводе открывающей сразу добавлять и закрывающую. И потом это пригодится в ситуации, когда вы быстро-быстро правите какой-то код, и по запарке после
else
